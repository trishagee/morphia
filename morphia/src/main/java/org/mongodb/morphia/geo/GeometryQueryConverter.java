...
public class GeometryQueryConverter extends TypeConverter<Geometry> implements SimpleValueConverter {
    ...
    @Override
    public Optional<BasicDBObject> encode(@NotNull final Geometry value, final MappedField optionalExtraInfo) {
        Object encode = getMapper().getConverters().encode(value);
        return Optional.of(new BasicDBObject("$geometry", encode));
    }
}
