...
public class QueryImpl<T> extends CriteriaContainerImpl implements Query<T> {
    ...
    public static BasicDBObject parseFieldsString(final String str, final Class clazz, final Mapper mapper, final boolean validate,
        ...
        for (String s : parts) {
            ...
            if (validate) {
                final ValidatedField validatedField = validateQuery(clazz, mapper, s, true);
                s = validatedField.getStoredFieldName();
            }
            ret = ret.add(s, dir);
        }
    ...
    public DBObject getFieldsObject() {
        ...
        for (String field : fields) {
            final ValidatedField validatedField = validateQuery(clazz, ds.getMapper(), field, validateName);
            field = validatedField.getStoredFieldName();
